...
class KITTIRoad(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            img = cv2.resize(_img, (int(_img.shape[0]), int(_img.shape[1])))   
            img = img.transpose(2, 0, 1)
            _lidar = cv2.resize(_lidar, (int(_img.shape[0]), int(_img.shape[1])))
            _lidar = _lidar.transpose(2, 0, 1)
        sample['image'] = img
        ...
        if self.do_semseg:
            ...
            if self.split == 'train':
                ...
                _semseg = cv2.resize(_semseg, (int(_img.shape[0]), int(_img.shape[1])), interpolation=cv2.INTER_NEAREST)
        ...
        if self.do_normals:
            ...
            if self.split == 'train':
                ...
                _normals = cv2.resize(_normals, (int(_img.shape[0]), int(_img.shape[1])), interpolation=cv2.INTER_NEAREST)     
        ...
        if self.do_depth:
            ...
            if self.split == 'train':
                ...
                _depth = cv2.resize(_depth, (int(_img.shape[0]), int(_img.shape[1])), interpolation=cv2.INTER_NEAREST)
            sample['depth'] = _depth
