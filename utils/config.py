...
def parse_task_dictionary(db_name, task_dictionary):
    ...
    task_cfg.NAMES = []
    task_cfg.NUM_OUTPUT = {}
    task_cfg.FLAGVALS = {'image': cv2.INTER_CUBIC, 'lidar': cv2.INTER_CUBIC}
    ...
    if 'include_normals' in task_dictionary.keys() and task_dictionary['include_normals']:
        # Surface Normals 
        tmp = 'normals'
        assert(db_name in ['PASCALContext', 'NYUD', 'KITTI', 'CITYSCAPES'])
        task_cfg.NAMES.append(tmp)
    ...
    if 'include_depth' in task_dictionary.keys() and task_dictionary['include_depth']:
        # Depth
        assert(db_name in['NYUD', 'KITTI', 'CITYSCAPES'])
        tmp = 'depth'
        task_cfg.NAMES.append(tmp)
        task_cfg.NUM_OUTPUT[tmp] = 1
        task_cfg.FLAGVALS[tmp] = cv2.INTER_NEAREST
...
def create_config(env_file, exp_file):
    ...
    cfg.ALL_TASKS.FLAGVALS = {'image': cv2.INTER_CUBIC, 'lidar': cv2.INTER_CUBIC}
    ...
    elif cfg['train_db_name'] == 'KITTI':
         cfg.TRAIN = edict()
         cfg.TRAIN.SCALE = (384, 1280)
         cfg.TEST = edict()
         cfg.TEST.SCALE = (384, 1280)
    
    elif cfg['train_db_name'] == 'CITYSCAPES':
         cfg.TRAIN = edict()
         cfg.TRAIN.SCALE = (1024, 2048)
         cfg.TEST = edict()
         cfg.TEST.SCALE = (1024, 2048)     
