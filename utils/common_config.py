...
def get_backbone(p):
    ...
    #     backbone_channels = [18, 36, 72, 144]
        ...
        from models.seg_hrnet import hrnet_w18
        backbone1, backbone2 = hrnet_w18(p['backbone_kwargs']['pretrained'])
        backbone_channels = [18, 36, 72, 144]   
    
    elif p['backbone'] == 'hrnet_w32':
        from models.seg_hrnet import hrnet_w32
        # from models.seg_hrnet_v02 import hrnet_w32
        backbone1, backbone2 = hrnet_w32(p['backbone_kwargs']['pretrained'], p['backbone_kwargs']['pretrained2'])
        backbone_channels = [32, 64, 128, 256]    
        
    elif p['backbone'] == 'hrnet_w48':
        from models.seg_hrnet import hrnet_w48
        # from models.seg_hrnet_v02 import hrnet_w48
        backbone1, backbone2 = hrnet_w48(p['backbone_kwargs']['pretrained'], p['backbone_kwargs']['pretrained2'])
        backbone_channels = [48, 96, 192, 384]
        
    elif p['backbone'] == 'hrnet_w64':
        from models.seg_hrnet import hrnet_w64
        backbone1, backbone2 = hrnet_w64(p['backbone_kwargs']['pretrained'])
    ...
    return backbone1, backbone2, backbone_channels
...
def get_head(p, backbone_channels, task):
    ...
    if p['head'] == 'deeplab':
        ...
        # from models.seg_hrnet_v02 import HighResolutionHead
...
def get_model(p):
    ...
    backbone1, backbone2, backbone_channels = get_backbone(p)
    ...
    if p['setup'] == 'single_task':
        ...
        # model = SingleTaskModel(backbone1, head, task)
        model = SingleTaskModel(backbone1, backbone2, head, task)
        ...
        if p['model'] == 'mti_net':
            ...
            heads = torch.nn.ModuleDict({task: get_head(p, backbone_channels, task) for task in p.TASKS.NAMES})
            model = MTINet(p, backbone1, backbone2, backbone_channels, heads)
...
def get_train_dataset(p, transforms):
    ...
    if db_name == 'PASCALContext':
        ...
        database = KITTIRoad(split='train', transform=transforms, do_edge='edge' in p.ALL_TASKS.NAMES, 
                                    do_semseg='semseg' in p.ALL_TASKS.NAMES, 
                                    do_normals='normals' in p.ALL_TASKS.NAMES, 
                                    do_depth='depth' in p.ALL_TASKS.NAMES, overfit=p['overfit'])
    elif db_name == 'CITYSCAPES':
        from data.cityscapes import CityScapes
        database = CityScapes(split='train', transform=transforms, do_edge='edge' in p.ALL_TASKS.NAMES, 
                                    do_semseg='semseg' in p.ALL_TASKS.NAMES, 
                                    do_normals='normals' in p.ALL_TASKS.NAMES, 
                                    do_depth='depth' in p.ALL_TASKS.NAMES, overfit=p['overfit'])    
...
def get_val_dataset(p, transforms):
    ...
    if db_name == 'PASCALContext':
        ...
        from data.kitti_road_v02 import KITTIRoad
        database = KITTIRoad(split='val', transform=transforms, do_edge='edge' in p.TASKS.NAMES, 
                                do_semseg='semseg' in p.TASKS.NAMES, 
                                do_normals='normals' in p.TASKS.NAMES, 
                                do_depth='depth' in p.TASKS.NAMES, overfit=p['overfit'])    
        ...
        from data.cityscapes import CityScapes
        database = CityScapes(split='val', transform=transforms, do_edge='edge' in p.ALL_TASKS.NAMES, 
                                    do_semseg='semseg' in p.ALL_TASKS.NAMES, 
                                    do_normals='normals' in p.ALL_TASKS.NAMES, 
                                    do_depth='depth' in p.ALL_TASKS.NAMES, overfit=p['overfit'])    
...
def get_loss(p, task=None):
    ...
    if task == 'edge':
        ...
        # from losses.loss_functions import BinaryCrossEntropyLoss
...
def get_criterion(p):
    ...
    if p['setup'] == 'single_task':
        ...
        elif p['loss_kwargs']['loss_scheme'] == 'mti_net_TrNet': # Fixed weights but at multiple scales
            from losses.loss_schemes import MTINet_TrNet_Loss
            loss_ft = torch.nn.ModuleDict({task: get_loss(p, task) for task in set(p.ALL_TASKS.NAMES)})
            loss_weights = p['loss_kwargs']['loss_weights']
            return MTINet_TrNet_Loss(p.TASKS.NAMES, p.AUXILARY_TASKS.NAMES, loss_ft, loss_weights)
