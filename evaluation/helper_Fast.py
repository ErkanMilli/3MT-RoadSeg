...
# def fast_hist(a, b, n, ignore_label=None, mask=None):
#     '''
# 	a and b are predict and mask respectively
# 	n is the number of classes
# 	'''
#     if mask is None:
#        mask = np.ones_like(a) == 1
#     k = (a >= 0) & (b < n)  & (a != ignore_label) & (mask.astype(np.bool))
...
def pxEval_maximizeFMeasure(conf_matrix):
    '''
    ...
    '''
    tag  = '_wp'
    prob_eval_scores = dict()
    if conf_matrix.sum() == 0:
        # return 0, 0, 0, 0, 0
        prob_eval_scores['MaxF'] = 0
        prob_eval_scores['FPR'+ tag] = 0
        prob_eval_scores['FNR'+ tag] = 0
        prob_eval_scores['PRE'+ tag] = 0
        prob_eval_scores['REC'+ tag] = 0
        return prob_eval_scores
    with np.errstate(divide='ignore',invalid='ignore'):
        classpre = np.diag(conf_matrix) / (conf_matrix.sum(0).astype(np.float) + 1e-10)
        classrecall = np.diag(conf_matrix) / (conf_matrix.sum(1).astype(np.float) + 1e-10)
        pre = classpre[1]
        recall = classrecall[1]
        F_score = 2 * (recall * pre) / (recall + pre + 1e-10)
        fpr = conf_matrix[0, 1] / (np.float(conf_matrix[0, 0] + conf_matrix[0, 1]) + 1e-10)
        fnr = conf_matrix[1, 0] / (np.float(conf_matrix[1, 0] + conf_matrix[1, 1]) + 1e-10)

    #ACC = (totalTP+ totalTN)/(totalPosNum+totalNegNum)
    # prob_eval_scores  = calcEvalMeasures(valuesMaxF)
    # prob_eval_scores['AvgPrec'] = AvgPrec
    prob_eval_scores['MaxF'] = F_score
    ...
    prob_eval_scores['FPR'+ tag] = fpr
    prob_eval_scores['FNR'+ tag] = fnr
    prob_eval_scores['PRE'+ tag] = pre
    prob_eval_scores['REC'+ tag] = recall
