        ...
        for task in p.TASKS.NAMES:
            ...
            for jj in range(int(inputs.size()[0])):
                if len(sample[task][jj].unique()) == 1 and sample[task][jj].unique() == 255:
                    continue
                fname = meta['image'][jj]   
                # output_task_s = output_task
                # output_task_s[output_task_s > 0] = 255
                # output_task_s = output_task
                ...
                result_d = cv2.resize(output_task[jj], dsize=(int(meta['im_size'][1][jj]), int(meta['im_size'][0][jj])))
                result_s = cv2.resize(output_task_s[jj], dsize=(int(meta['im_size'][1][jj]), int(meta['im_size'][0][jj])))
                # result_n = cv2.resize(output_task[jj], dsize=(int(meta['im_size'][1][jj]), int(meta['im_size'][0][jj])))
                if task == 'depth':
                    sio.savemat(os.path.join(save_dirs[task], fname + '.mat'), {'depth': result_d})
                # elif task == 'normal':
                #     imageio.imwrite(os.path.join(save_dirs[task], fname + '.png'), result_n.astype(np.uint8))
                else:
                    # result = result * 255.
                    # result[result > 255] = 255
                    imageio.imwrite(os.path.join(save_dirs[task], fname + '.png'), result_s.astype(np.uint8))
