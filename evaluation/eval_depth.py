...
def eval_depth(loader, folder):
    ...
    for i, sample in enumerate(loader):
        ...
        filename = os.path.join(folder, im_name_splits[0] + '_road_' + im_name_splits[1] + '.mat')
        ...
        valid_mask = (label != 0)
        # valid_mask = (label != 0) & (label != np.inf)
...
def eval_depth_predictions(database, save_dir, overfit=False):
    ...
    if database == 'NYUD':
        ...
        db = NYUD_MT(split=gt_set, do_depth=True, overfit=overfit)
         ...
         gt_set = 'val'
         db = KITTIRoad(split=gt_set, do_depth=True, overfit=overfit)
         
    elif database == 'CITYSCAPES':
         from data.cityscapes import CityScapes 
         gt_set = 'val'
         db = CityScapes(split=gt_set, do_depth=True, overfit=overfit)     
