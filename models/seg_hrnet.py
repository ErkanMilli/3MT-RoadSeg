...
# Minor changes made by Erkan Milli
# ------------------------------------------------------------------------------
...
def hrnet_w18(pretrained=False):
    ...
    model1 = HighResolutionNet(hrnet_cfg)
    model2 = HighResolutionNet(hrnet_cfg)
    if pretrained:
        pretrained_weights = os.path.join(PROJECT_ROOT_DIR, 'models', 'pretrained_models', 'hrnet_w18_small_model_v2.pth')
        if os.path.exists(pretrained_weights):
            model1.init_weights(pretrained_weights)
        else:
            raise AssertionError('Error: No pretrained weights found for HRNet18. \n Download weights from https://github.com/HRNet/HRNet-Image-Classification and save them to {}'.format(pretrained_weights))
    
    return model1, model2        
            
def hrnet_w32(pretrained=False, pretrained2=False):
    import yaml
    from utils.mypath import PROJECT_ROOT_DIR
    hrnet_cfg = os.path.join(PROJECT_ROOT_DIR, 'models', 'model_info', 'hrnet_w32.yml')
    ...
    model2 = HighResolutionNet(hrnet_cfg)
    if pretrained:
        pretrained_weights = os.path.join(PROJECT_ROOT_DIR, 'models', 'pretrained_models', 'hrnetv2_w32_imagenet_pretrained.pth')
        if os.path.exists(pretrained_weights):
            model1.init_weights(pretrained_weights)
        else:
    ...
    if pretrained2:
        pretrained_weights = os.path.join(PROJECT_ROOT_DIR, 'models', 'pretrained_models', 'hrnetv2_w32_imagenet_pretrained.pth')
        if os.path.exists(pretrained_weights):
            model2.init_weights(pretrained_weights)
        else:
            raise AssertionError('Error: No pretrained weights found for HRNet48. \n Download weights from https://github.com/HRNet/HRNet-Image-Classification and save them to {}'.format(pretrained_weights))    
    return model1, model2            
            
def hrnet_w48(pretrained=False, pretrained2=False):
    import yaml
    from utils.mypath import PROJECT_ROOT_DIR
    ...
    model2 = HighResolutionNet(hrnet_cfg)
    if pretrained:
        pretrained_weights = os.path.join(PROJECT_ROOT_DIR, 'models', 'pretrained_models', 'hrnetv2_w48_imagenet_pretrained.pth')
        if os.path.exists(pretrained_weights):
            model1.init_weights(pretrained_weights)
        else:
            raise AssertionError('Error: No pretrained weights found for HRNet48. \n Download weights from https://github.com/HRNet/HRNet-Image-Classification and save them to {}'.format(pretrained_weights))   
...
def hrnet_w64(pretrained=False, pretrained2=False):
    import yaml
    from utils.mypath import PROJECT_ROOT_DIR
    hrnet_cfg = os.path.join(PROJECT_ROOT_DIR, 'models', 'model_info', 'hrnet_w64.yml')
    ...
    model2 = HighResolutionNet(hrnet_cfg)
    if pretrained:
        pretrained_weights = os.path.join(PROJECT_ROOT_DIR, 'models', 'pretrained_models', 'hrnetv2_w64_imagenet_pretrained.pth')
        if os.path.exists(pretrained_weights):
            model1.init_weights(pretrained_weights)
        else:
            raise AssertionError('Error: No pretrained weights found for HRNet64. \n Download weights from https://github.com/HRNet/HRNet-Image-Classification and save them to {}'.format(pretrained_weights))            
